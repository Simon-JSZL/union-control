...
public class ScheduledTaskServiceImpl implements ScheduledTaskService {
    ...
    public Map<String, Object> resolveScheduledToken(String rawToken) {
        return mapper.resolveScheduledToken(ScheduledExecutionToken.hashSubmitted(rawToken));
    }
        ...
        for (int index = 0; index < CRON_INTERVAL_CHECKS; index++) {
            ...
            if (Duration.between(previous.toInstant(), next.toInstant()).getSeconds()
                ...
                throw new IllegalArgumentException(
                        "cronExpression 触发间隔不能小于 " + MINIMUM_INTERVAL_SECONDS + " 秒");
            previous = next;
        }
    }
    ...
    Instant nextInterval(Instant anchor, long intervalSeconds, Instant after) {
        if (intervalSeconds < MINIMUM_INTERVAL_SECONDS)
            throw new IllegalArgumentException(
                    "intervalSeconds 不能小于 " + MINIMUM_INTERVAL_SECONDS);
    ...
    static String databaseDateTime(Instant value) {
        return value == null ? null : DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS")
